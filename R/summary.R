...
summary.mgwnbr_model <- function(x, ...){
  #check if object is of class mgwnbr
  if (!inherits(x, "mgwnbr_model")) {
    stop("Object is not of class 'mgwnbr_model'")
  }
  summary_list <- list()
  #general bandwidth
  summary_list$general_bandwidth <- x$general_bandwidth
  #covariate bandwidths
  if (!is.null(x$band)){
    summary_list$bandwidths <- unlist(x$band[nrow(x$band), -ncol(x$band)])
  }
  #enp and t test info
  summary_list$test_info <- rbind(x$ENP, x$alpha_level_5_pct, x$t_critical)
  rownames(summary_list$test_info) <- c("ENP", "alpha_level_5_pct", "t_critical")
  #5 number summary for estimates
  summary_list$estimates <- x$mgwr_param_estimates
  ...
  summary_list$std_errors <- x$mgwr_se
  ...
  summary_list$deviance <- round(as.numeric(x$measures[1]), 4)
  summary_list$full_log_likelihood <- round(as.numeric(x$measures[2]), 4)
  summary_list$aic <- round(as.numeric(x$measures[5]), 4)
  summary_list$aicc <- round(as.numeric(x$measures[6]), 4)
  #list
